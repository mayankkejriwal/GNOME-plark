...
def drop_sonobuoy_in_two_layer(current_gameboard, sonobuoy_name, location_name, decision='Shallow'):
    """
    ...
    for name, weapon in current_gameboard['weapons_inventory'].items():
        if name == sonobuoy_name:
            weapon.attributes['two_layer'] = decision
            print(f'Setting sonobuoy {sonobuoy_name} with depth {weapon.attributes["two_layer"]}')
            break


def panther_decision_in_two_layer(player, current_gameboard, panther_path, decision='Shallow'):
    """
    ...
    # key is "two_layer" and only for panther
    if player.player_class == 'Panther':
        print('Panther made additional decision for two_layer')
        player.additional_attributes['two_layer'] = decision
        player.additional_attributes_history.append(decision)  # add to history
        print(f'{player.player_name} decides its depth to be {decision}')
...
def sonobuoy_consider_two_layer(current_gameboard, s_updates, sonobuoy_panther_found, weapon, plark_location, player, det_range):
    ...
    if len(depth_hist) >= 2 and depth_hist[-1] != depth_hist[-2]:  # plark changes its depth currently
        ...
        weapon_depth = weapon.attributes['two_layer']
        plark_depth = player.additional_attributes['two_layer']
